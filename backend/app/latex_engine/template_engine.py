...
from .sanitizer import escape_latex
...
def find_tex(tdir: Path) -> Path | None:
...
def find_pdf(tdir: Path) -> Path | None:
...
def scan_templates():
    ...
    for tid, meta in TEMPLATE_META.items():
        ...
        if tdir.exists():
            tex = find_tex(tdir)
            entry["has_preview"] = find_pdf(tdir) is not None
            if tex:
...
def assert_template(template_id: str) -> tuple[Path, str]:
...
def _generate_latex_classic(data: dict, config: dict) -> str:
    ...
    p = data.get("personal", {})
    name = escape_latex(p.get("name", "Your Name"))
    email = escape_latex(p.get("email", ""))
    phone = escape_latex(p.get("phone", ""))
    linkedin = escape_latex(p.get("linkedin", ""))
    website = escape_latex(p.get("website", ""))
    address = escape_latex(p.get("address", ""))
...
{{\\LARGE \\textbf{{{name}}}}}"""
    ...
    if skills:
        escaped_skills = [escape_latex(s) for s in skills]
    ...
    for exp in data.get("experience", []):
        ...
        sd = escape_latex(exp.get("start_date", ""))
        ed = escape_latex(exp.get("current", False) and "Present" or exp.get("end_date", ""))
    ...
    if edu_data:
        ...
        for edu in edu_data:
            ...
            sd = escape_latex(edu.get("start_date", ""))
            ed = escape_latex(edu.get("end_date", ""))
    ...
    if proj_data:
        ...
        for proj in proj_data:
            ...
            if pdesc:
                tex += f"{pdesc}\n"
            if ptech:
                tex += f"\\textit{{Technologies: {', '.join(escape_latex(t) for t in ptech)}}}\n"
            tex += "\\vspace{4pt}\n\n"
    ...
    if lang_data:
        ...
        for l in lang_data:
            lang = escape_latex(l.get("language", ""))
            prof = escape_latex(l.get("proficiency", ""))
    ...
    if pub_data:
        ...
        for p in pub_data:
            ptitle = escape_latex(p.get("title", ""))
            venue = escape_latex(p.get("venue", ""))
            year = escape_latex(p.get("year", ""))
    ...
    if award_data:
        tex += "\\section*{Awards}\n\\begin{itemize}\n"
        for a in award_data:
            aname = escape_latex(a.get("name", ""))
            issuer = escape_latex(a.get("issuer", ""))
            year = escape_latex(a.get("year", ""))
            parts = [aname]
            if issuer:
                parts.append(issuer)
            if year:
...
def _generate_latex_modern(data: dict, config: dict) -> str:
    ...
    website = escape_latex(p.get("website", ""))
    address = escape_latex(p.get("address", ""))
    title_str = escape_latex(p.get("title", ""))
    summary = escape_latex(data.get("summary", ""))
    ...
    if skills:
        ...
        tex_lines.append(", ".join(escape_latex(s) for s in skills))
    if experience:
        tex_lines.append("")
        ...
        for exp in experience:
            ...
            bullet_str = " \\\\ ".join(escape_latex(b) for b in bullets[:2])
            tex_lines.append("")
...
\\begin{{center}}"""
    ...
    if skills:
        ...
        escaped = [escape_latex(s) for s in skills]
        tex += ", ".join(escaped) + "\n\n"
    ...
    for exp in data.get("experience", []):
        ...
        for b in exp.get("bullets", []):
            tex += f"\\textbullet\\ {escape_latex(b)}\\\\\n"
        tex += "\\vspace{4pt}\n\n"
        break
...
def _generate_latex_sidebar(data: dict, config: dict) -> str:
    ...
    if experience:
        ...
        for exp in experience:
            ...
            for b in exp.get("bullets", []):
                tex_lines.append("\\textbullet\\ " + escape_latex(b))
    edu_data = data.get("education", [])
    if edu_data:
        tex_lines.append("\\cvsection{Education}")
        for edu in edu_data:
    ...
    if skills:
        ...
        tex_lines.append("\\hrule\\vspace{8pt}")
        for s in skills[:10]:
            tex_lines.append("\\textcolor{white}{\\textbullet\\ " + escape_latex(s) + "}\\\\[4pt]")
    tex_lines.append("\\end{minipage}}")
    tex_lines.append("")
    tex_lines.append("\\null\\vspace{\\fill}")
...
def generate_latex(data: dict, template_id: str, config: dict) -> str:
...
def analyze_jd(job_description: str, company_name: str = "", job_title: str = "") -> dict:
    from app.groq_helper import analyze_jd_with_groq
...
def optimize_resume_data(resume_data: dict, analysis: dict) -> dict:
